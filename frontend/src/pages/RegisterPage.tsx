...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(detail ? t(`errors.${detail}`, { defaultValue: detail }) : t("auth.error_register_failed"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-fs-25 font-sans">
      ...
      <div className="w-full max-w-md px-4">
        ...
        <div className="bg-white rounded-[14px] border border-fs-200 p-8 space-y-5">
          <h2 className="text-base font-semibold text-fs-900">{t("auth.register")}</h2>
          ...
          {pending && (
            <div className="text-sm text-[#C68A1A] bg-[#FFF6E0] border border-[#C68A1A]/30 px-3 py-2.5 rounded-lg">
              {t("auth.register_success")}
              <br />
              <Link to="/login" className="text-feents-700 hover:text-feents font-medium mt-1 inline-block">
                {t("auth.go_to_login")}
              </Link>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-fs-600 uppercase tracking-wide mb-1.5">{t("auth.password_confirm")}</label>
              <input
                type="password"
                className="w-full border border-fs-200 rounded-lg px-3 py-2.5 text-sm text-fs-900 placeholder:text-fs-400 focus:outline-none focus:border-feents focus:ring-2 focus:ring-feents/20 transition-colors"
                value={confirm}
          ...
          <p className="text-xs text-center text-fs-400">
            {t("auth.have_account")}{" "}
            <Link to="/login" className="text-feents-700 hover:text-feents font-medium">
              {t("auth.login")}
            </Link>
          </p>
        </div>
